...
export default function Navbar() {
  ...
  return (
    <nav style={{
      ...
      background: scrolled ? 'rgba(4,8,26,0.95)' : 'rgba(4,8,26,0.7)',
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 24px', height: 68, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
          <div style={{ width: 36, height: 36, background: 'linear-gradient(135deg,#1e40af,#7c3aed)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>⚡</div>
          <span style={{ fontWeight: 800, fontSize: 18, background: 'linear-gradient(135deg,#60a5fa,#a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>AgentMatch AI</span>
        </Link>
        <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
          {[{ href: '/agents', label: 'Agents' }, { href: '/missions', label: 'Missions' }, { href: '/pricing', label: 'Tarifs' }].map(({ href, label }) => (
            <Link key={href} href={href} style={{ padding: '8px 16px', borderRadius: 8, fontSize: 14, fontWeight: 500, textDecoration: 'none', color: pathname.startsWith(href) ? '#60a5fa' : '#94a3b8', background: pathname.startsWith(href) ? 'rgba(37,99,235,0.1)' : 'transparent' }}>{label}</Link>
          ))}
        </div>
          ...
          {user ? (
            <>
              ...
              <div style={{ position: 'relative' }}>
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,#1e40af,#7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 14, cursor: 'pointer' }} onClick={() => setMenuOpen(!menuOpen)}>{displayName.charAt(0).toUpperCase()}</div>
                {menuOpen && (
            ...
            <>
              ...
              <Link href="/register/agent" style={{ padding: '10px 18px', borderRadius: 10, background: 'linear-gradient(135deg,#1e40af,#7c3aed)', color: 'white', textDecoration: 'none', fontSize: 14, fontWeight: 600 }}>Commencer</Link>
            </>
          )}
        </div>
      </div>
